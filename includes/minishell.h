...
/*emulated comms*/
int							find_string_in_array(char **array, const char *str,
								size_t n);
...
void						execute_exit(t_minishell *minishell, int fd,
								t_cmd *cur);
void						execute_command(t_minishell *minishell, int fd,
								t_cmd *cur);
// exe
...
void						open_file(t_minishell *minishell,
								const char *filename, int perm_or_no_file);
...
void						redirects(t_minishell *minishell, t_cmd *current,
								int i, int num_cmd);
void						handle_hd(t_cmd *current, int *last_input_fd);
void						handle_output_file(t_minishell *minishell,
								t_list *tmp, int *last_output_fd, int flags);
void						handle_input_file(t_minishell *minishell,
								t_list *tmp, int *last_input_fd);
...
char						*construct_error(char *arg1, char *arg2, char *arg3,
								char mode);
void						not_found(t_minishell *minishell, char *cmd);
void						no_file(t_minishell *minishell, char *filename);
void						no_path_file(t_minishell *minishell,
								char *filename);
int							syntax_error(t_minishell *minishell, char *token);
void						arg_count_error(t_minishell *minishell,
								char *command);
void						exe_binary_error(t_minishell *minishell,
								char *path);
...
void						numeric_error(t_minishell *minishell, char *cmd,
								char *arg);
void						deleted_dir(t_minishell *minishell);
void						handle_incorrect_arguments(t_minishell *minishell,
								char *av);
// path.c
